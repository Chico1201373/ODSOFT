package pt.psoft.g1.psoftg1.externalapimanagement.whiteboxtests.service;
...
class ExternalApiServiceGoogleWhiteBoxTest_IT {
    ...
    @Test
    void serviceReturnsIsbn13WhenGoogleReturnsIsbn13() {
        String json = "{" +
                "\"items\": [ { \"volumeInfo\": { \"industryIdentifiers\": [ { \"type\": \"ISBN_13\", \"identifier\": \"9784444444444\" } ] } } ] }";

        server.expect(once(), requestTo(org.hamcrest.Matchers.containsString("intitle:Some%20Title")))
                .andRespond(withSuccess(json, org.springframework.http.MediaType.APPLICATION_JSON));

        String isbn = externalApiService.getIsbn("Some Title");
        assertThat(isbn, is("9784444444444"));
        server.verify();
    }

    @Test
    void serviceFallsBackToIsbn10WhenIsbn13Missing() {
        String json = "{" +
                "\"items\": [ { \"volumeInfo\": { \"industryIdentifiers\": [ { \"type\": \"ISBN_10\", \"identifier\": \"9999999999\" } ] } } ] }";

        server.expect(once(), requestTo(org.hamcrest.Matchers.containsString("intitle:Other%20Title")))
                .andRespond(withSuccess(json, org.springframework.http.MediaType.APPLICATION_JSON));

        String isbn = externalApiService.getIsbn("Other Title");
        assertThat(isbn, is("9999999999"));
        server.verify();
    }

    @Test
    void serviceReturnsNullWhenGoogleThrows() {
